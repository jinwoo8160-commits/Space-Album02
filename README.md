...
| 줌 | 레이어 |
| --- | --- |
| **0.0 ~ 9.5** | 육지 도트. 사진 1장당 중심에서 유클리드 거리 `d` 가 격자 4.5칸 이내인 이웃에 `e^(-(d/σ)²)` 가중치를 더 해 원형으로 퍼집니다. `totalScore > 0` 인 도트만 4분위 `densityLevel`(25% 단위, 불투명도 1.0 / 0.75 / 0.5 / 0.25)로 진하기를 나눕니다. 줌 중에는 재계산하지 않습니다. |
